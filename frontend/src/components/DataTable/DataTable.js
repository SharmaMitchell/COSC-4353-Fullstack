...
function DataTable() {
  ...
  const formatDate = (dateString) => {
    ...
    return `${theDate.getMonth() + 1}/${theDate.getDate()}/${theDate.getFullYear()}, ${formatTime(theDate)}`;
  }
  return (
        ...
        <TableBody>
          {tableData.map((row, index) => (
            ...
            >
              ...
              <TableCell>{row.gallonsRequested.toLocaleString('en-US')}</TableCell>
              <TableCell>{row.deliveryAddress}</TableCell>
              <TableCell>{formatDate(row.deliveryDate)}</TableCell>
              <TableCell>${parseFloat(row.suggestedPrice).toFixed(2).toLocaleString('en-US')}</TableCell>
              <TableCell>${parseFloat(row.quote).toFixed(2).toLocaleString('en-US')}</TableCell>
            </StyledTableRow>
          ))}
        </TableBody>
      </Table>
